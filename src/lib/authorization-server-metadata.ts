...
/**
 * Validates the shape of an Authorization Server Metadata response from an untrusted
 * server (SEC-12). `issuer` must be a string; a malformed scopes_supported is dropped
 * rather than trusted, since it is later joined into the requested scope.
 */
...
export async function fetchAuthorizationServerMetadata(serverUrl: string): Promise<AuthorizationServerMetadata | undefined> {
  ...
  try {
    ...
    // RFC 8414 §3.3 issuer binding (SEC-13): the issuer should match the origin the
    // metadata was fetched from. Warn rather than hard-fail, to avoid breaking
    // legitimately-configured-but-sloppy servers while still surfacing the
    // token-confusion risk.
    try {

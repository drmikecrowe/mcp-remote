...
export class NodeOAuthClientProvider implements OAuthClientProvider {
  ...
  /**
   * Selects the RFC 8707 resource indicator. The SDK calls this from selectResourceURL
   * and sends the result on BOTH the authorization request and the token request, so
   * the audience of the issued token stays bound to the resource the user consented to.
   * Setting `resource` only on the authorization URL would leave the token exchange
   * requesting a different audience than the one that was authorized.
  ...
  async validateResourceURL(serverUrl: string | URL, resource?: string): Promise<URL | undefined> {
    ...
    // No override: mirror the SDK default. Only send a resource indicator when the
    // server publishes Protected Resource Metadata, and require it to match the server.
    if (!resource) {
      return undefined
    }

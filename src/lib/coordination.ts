...
export async function isLockValid(lockData: LockfileData): Promise<boolean> {
  ...
  try {
    ...
    // service, or a planted lockfile pointing at an attacker's server (issue #6).
...
export async function waitForAuthentication(port: number, expectedSecret?: string): Promise<boolean> {
  ...
  // Overall deadline so a wedged or vanished primary can't hang us forever
  // (issue #6). Generous, because the primary may be waiting on interactive
  // browser auth which can legitimately take minutes.
  ...
  try {
    ...
    while (true) {
      ...
      try {
        ...
        // Verify we're still talking to the expected auth instance (issue #6)
        if (expectedSecret) {
          const responseSecret = response.headers.get('x-mcp-auth-secret')
...
export async function coordinateAuth(
  ...
  if (lockData && (await isLockValid(lockData))) {
    ...
    try {
      ...
      if (authCompleted) {
        ...
        const dummyServer = express().listen(0, '127.0.0.1') // Listen on any available loopback port (issue #4)
  ...
  // Per-instance secret so peers can confirm they're polling the genuine primary (issue #6)
